  ...
  return <CalculatorShell><CalculatorCard><PageHeading title="Proposta Conexa" description="Escolha o plano e configure produtos, implantação e descontos em uma única tela." />
    ...
    <div className="space-y-8 p-5 md:p-6">
      ...
      <section id="produtos" className="scroll-mt-36 border-t border-gray-100 pt-7 dark:border-gray-800">
        ...
        <div className="space-y-4">{store.resources.map((resource) => <div key={resource.id} className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><div className="grid items-center gap-3 sm:grid-cols-[1fr_1fr_140px_36px]"><input className={ui.input} aria-label="Nome do produto" placeholder="Nome do produto" value={resource.name} onChange={(event) => store.updateResource({ ...resource, name: event.target.value })} /><CurrencyInput label={`Valor de ${resource.name || 'produto'}`} value={resource.value} onChange={(value) => store.updateResource({ ...resource, value })} /><div className="relative"><span className="pointer-events-none absolute inset-y-0 left-3 z-10 flex items-center text-[10px] font-semibold uppercase tracking-wider text-gray-400">Qtd.</span><NumberInput label={`Quantidade de ${resource.name || 'produto'}`} className="pl-12 text-center" integer min={1} zeroPlaceholder value={resource.quantity} onChange={(quantity) => store.updateResource({ ...resource, quantity })} /></div><button type="button" aria-label={`Excluir ${resource.name || 'produto'}`} onClick={() => store.removeResource(resource.id)} className={ui.dangerIconButton}><Trash2 size={17} /></button></div><div className="mt-3"><DiscountFields title={`Desconto de ${resource.name || 'produto'}`} type={resource.discountType} value={resource.discountValue} currency={store.currency} onTypeChange={(discountType) => store.updateResource({ ...resource, discountType, discountValue: 0 })} onValueChange={(discountValue) => store.updateResource({ ...resource, discountValue })} /></div></div>)}{!store.resources.length && <p className="rounded-xl border border-dashed border-gray-200 py-6 text-center text-xs text-gray-400 dark:border-gray-800">Nenhum produto adicionado</p>}</div>
      </section>

      <section id="servicos" className="scroll-mt-36 border-t border-gray-100 pt-7 dark:border-gray-800"><div className="mb-4 flex items-center justify-between gap-4"><div><h3 className="font-semibold text-gray-900 dark:text-white/90">Implantação</h3><p className="mt-1 text-xs text-gray-400">Adicione os itens de implantação, treinamento, Setup ou configuração.</p></div><button type="button" onClick={store.addService} className={ui.compactCreateButton}><Plus size={16} className="shrink-0" />Adicionar implantação</button></div><div className="space-y-4">{store.services.map((service) => <div key={service.id} className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><div className="grid gap-3 sm:grid-cols-[1fr_1fr_36px]"><input className={ui.input} aria-label="Nome da implantação" placeholder="Ex.: Implantação, treinamento ou Setup" value={service.name} onChange={(event) => store.updateService({ ...service, name: event.target.value })} /><CurrencyInput label={`Valor de ${service.name || 'implantação'}`} value={service.value} onChange={(value) => store.updateService({ ...service, value })} /><button type="button" aria-label={`Excluir ${service.name}`} onClick={() => store.removeService(service.id)} className={ui.dangerIconButton}><Trash2 size={17} /></button></div><div className="mt-3"><DiscountFields title={`Desconto de ${service.name || 'implantação'}`} type={service.discountType} value={service.discountValue} currency={store.currency} onTypeChange={(discountType) => store.updateService({ ...service, discountType, discountValue: 0 })} onValueChange={(discountValue) => store.updateService({ ...service, discountValue })} /></div></div>)}{!store.services.length && <p className="rounded-xl border border-dashed border-gray-200 py-6 text-center text-xs text-gray-400 dark:border-gray-800">Nenhuma implantação adicionada</p>}</div>{store.services.length > 0 && <div className="mt-5 border-t border-gray-100 pt-5 dark:border-gray-800"><div className="sm:max-w-sm"><label className={ui.label}>Parcelamento total da implantação</label><AnimatedSelect aria-label="Parcelamento total da implantação" value={store.implementationInstallments} onChange={(event) => store.setImplementationInstallments(Number(event.target.value))}>{[1, 2, 3, 4, 5].map((installment) => <option key={installment} value={installment}>{installment}x</option>)}</AnimatedSelect><p className="mt-2 text-xs text-gray-400">Aplicado ao valor total de todas as implantações.</p></div></div>}</section>
    </div>
