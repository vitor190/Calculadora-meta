...
export function CostDistributionChart({ items, currency }: { items: ChartItem[]; currency: CurrencyCode }) {
    ...
    <article className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 md:p-6">
      ...
      <div className="mt-6 flex flex-col items-center gap-7 sm:flex-row">
        <div className="relative h-44 w-44 shrink-0 rounded-full" style={{ background: total ? `conic-gradient(${gradient})` : 'conic-gradient(#e4e7ec 0deg 360deg)' }}>
          <div className="absolute inset-7 flex flex-col items-center justify-center rounded-full bg-white dark:bg-gray-900"><span className="text-xs text-gray-400">Subtotal</span><strong className="mt-1 text-center text-base text-gray-900 dark:text-white/90">{formatCurrency(total, currency)}</strong></div>
        </div>
        <div className="w-full space-y-3">
          {items.map((item) => <div key={item.label} className="flex items-center justify-between gap-4 text-sm"><span className="flex min-w-0 items-center gap-2 text-gray-500 dark:text-gray-400"><i className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.color }} /><span className="truncate">{item.label}</span></span><strong className="text-gray-800 dark:text-white/90">{formatCurrency(item.value, currency)}</strong></div>)}
        </div>
      </div>
    </article>
  );
