  ...
  return <CalculatorShell>
    ...
    <section className="mt-5 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="divide-y divide-gray-100 px-5 dark:divide-gray-800">
        ...
        <DetailGroup label="Plano Conexa" total={formatCurrency(totals.discountedPlan, store.currency)} color="#32a699"><DetailRow label={selectedPlan?.name ?? 'Plano selecionado'} helper="Valor mensal" value={formatCurrency(store.planValue, store.currency)} discount={totals.planDiscount > 0 ? `− ${formatCurrency(totals.planDiscount, store.currency)} de desconto` : undefined} /></DetailGroup>
