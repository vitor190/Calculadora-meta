  ...
  return (
    <CalculatorShell>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Descontos" value={formatCurrency(totals.discount, store.currency)} helper="Plano mensal + serviços extras" icon={BadgePercent} tone="warning" />
      ...
      <section className="mt-5 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="divide-y divide-gray-100 px-5 dark:divide-gray-800">
          ...
          {totals.planDiscount > 0 && <div className="flex items-center justify-between py-3.5 text-sm text-brand-600 dark:text-brand-400"><span>Desconto do plano mensal</span><strong>− {formatCurrency(totals.planDiscount, store.currency)}</strong></div>}{totals.serviceDiscount > 0 && <div className="flex items-center justify-between py-3.5 text-sm text-brand-600 dark:text-brand-400"><span>Descontos dos serviços extras</span><strong>− {formatCurrency(totals.serviceDiscount, store.currency)}</strong></div>}
        </div>
