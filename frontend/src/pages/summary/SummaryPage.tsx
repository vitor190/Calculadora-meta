...
export function SummaryPage() {
  ...
  const openProposal = () => {
    const proposal = {
      ...
      implementationInstallments: store.implementationInstallments,
  ...
  return <CalculatorShell>
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="divide-y divide-gray-100 px-5 dark:divide-gray-800">
        ...
        <DetailGroup label="Implantação" total={formatCurrency(totals.discountedServices, store.currency)} color="#f04438">{store.services.length ? <>{store.services.map((item) => { const discount = discountAmount(item.value, item.discountType, item.discountValue); const netValue = item.value - discount; return <DetailRow key={item.id} label={item.name || 'Implantação sem nome'} value={formatCurrency(item.value, store.currency)} helper={discount > 0 ? `Valor com desconto: ${formatCurrency(netValue, store.currency)}` : undefined} discount={discount > 0 ? `− ${formatCurrency(discount, store.currency)} de desconto` : undefined} />; })}<DetailRow label="Parcelamento total" helper="Aplicado ao total da implantação" value={store.implementationInstallments > 1 ? `${store.implementationInstallments}x de ${formatCurrency(totals.implementationTotal / store.implementationInstallments, store.currency)}` : 'Pagamento único'} /></> : <EmptyDetail>Nenhuma implantação adicionada.</EmptyDetail>}</DetailGroup>
      </div>
      <div className="grid md:grid-cols-2"><div className="flex items-center justify-between gap-4 bg-brand-500 px-5 py-5 text-white"><div><p className="text-xs font-medium uppercase tracking-wider text-white/70">Total mensal</p><p className="mt-1 text-sm text-white/80">Meta, plano e produtos</p></div><strong className="text-xl font-semibold sm:text-2xl">{formatCurrency(totals.recurringTotal, store.currency)}</strong></div><div className="flex items-center justify-between gap-4 bg-[#087f8c] px-5 py-5 text-white"><div><p className="text-xs font-medium uppercase tracking-wider text-white/70">Implantação</p><p className="mt-1 text-sm text-white/80">{store.services.length === 0 ? 'Nenhum valor adicionado' : store.implementationInstallments > 1 ? `${store.implementationInstallments}x de ${formatCurrency(totals.implementationTotal / store.implementationInstallments, store.currency)}` : 'Pagamento único'}</p></div><strong className="text-xl font-semibold sm:text-2xl">{formatCurrency(totals.implementationTotal, store.currency)}</strong></div></div>
