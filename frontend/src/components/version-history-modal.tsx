  ...
  return (
    <div className="fixed inset-0 z-[10000001] flex items-center justify-center p-4">
      ...
      <section
        ...
        className="relative z-10 max-h-[85vh] w-full max-w-2xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xl dark:border-gray-800 dark:bg-gray-900"
      >
        <header className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Evolução da Calculadora Conexa</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="custom-scrollbar max-h-[calc(85vh-74px)] overflow-y-auto p-5 md:p-6">
          {versions.map((item) => (
            <article
              key={item.version}
              className="relative border-l-2 border-brand-100 pl-6 dark:border-brand-500/20"
            >
              ...
              <div className="space-y-2">
                {item.changes.map(({ icon: Icon, text }) => (
                  <div
                    key={text}
                    className="flex gap-3 rounded-lg bg-gray-50 p-3 text-sm text-gray-600 dark:bg-gray-800 dark:text-gray-300"
                  >
                    <Icon
                      size={16}
